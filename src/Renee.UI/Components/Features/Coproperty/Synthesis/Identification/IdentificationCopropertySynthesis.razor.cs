...
using Renee.UI.Components.Features.Coproperty.Synthesis.Identification.ViewModel;
using Radzen;
...
using Renee.UI.Components.Features.Coproperty.Synthesis.Identification.Modal;
...
namespace Renee.UI.Components.Features.Coproperty.Synthesis.Identification;

public partial class IdentificationCopropertySynthesis
{
    ...
    public IdentificationCopropertySynthesisViewModel IdentificationViewModel { get; set; } = new();
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        var result = await CopropertyProfileService.GetCopropertyProfileForIdentificationMilestoneById(CopropertyProfileId, UserId, UserRole);
        ...
        if (CopropertyIdentificationDto != null)
        {

            IdentificationViewModel = IdentificationCopropertySynthesisViewModel.CreateViewModelFromDto(CopropertyIdentificationDto);

            SynthesysLayoutStateManager.AssociatedResourceReference = IdentificationViewModel.Reference;
            ...
            SynthesysLayoutStateManager.ShouldDisplayNavigationButton = !NavigationHistoryManager.PreviousUri!.Contains(Endpoints.CopropertyIdentification);
            SynthesysLayoutStateManager.IsAccompanyingFile = false;

            SynthesysLayoutStateManager.NotifyStateChanged();

            _blobName = $"{IdentificationViewModel.Reference}_{Labels.AuditReportFile}";
    ...
    private string GetEnumDescription(Enum? value) => EnumHelper.GetDescription(value);
    ...
    {
        if (NavigationHistoryManager.PreviousUri!.Contains(Endpoints.CopropertyIdentification))
            return true;

        return IdentificationViewModel.Stage >= AccompanyingFileStage.Identify;
    }
    ...
    private async Task OnSubmitSynthesis()
    {
        ...
        var synthesisCopropertyValidationModalViewModel = new IdentifyCopropertySynthesisValidationModalViewModel
        {
            CopropertyProfileId = CopropertyProfileId,
            UserRole = role.Value,
            IsInTzeeProgram = IdentificationViewModel.IsInTzeeProgram
        }; 

        ModalService.Show<IdentifyCopropertySynthesisValidationModal>(
            new ModalParameters
            {
                { nameof(IdentifyCopropertySynthesisValidationModal.ViewModel), synthesisCopropertyValidationModalViewModel },
                { nameof(IdentifyCopropertySynthesisValidationModal.FileName), _fileName },
                { nameof(IdentifyCopropertySynthesisValidationModal.MemoryStream), _memoryStreamFile }
            },
    ...
    private bool ShouldDisplaySubmissionButton() => IdentificationViewModel.Stage == AccompanyingFileStage.Identify && (IdentificationViewModel.Status == AccompanyingFileStatus.InProgress || IdentificationViewModel.Status == AccompanyingFileStatus.Rejected);

    private bool IsAccompanyingFileWaitingForValidation() => IdentificationViewModel.Status == AccompanyingFileStatus.WaitingForApproval && IdentificationViewModel.Stage == AccompanyingFileStage.Identify;
    ...
    (
            UserId == IdentificationViewModel.SolidarBuilderId ||
            UserId == IdentificationViewModel.SecondSolidarBuilderId ||
            UserId == IdentificationViewModel.DiffuseCoordinatorId ||
            UserId == IdentificationViewModel.TargetedCoordinatorId ||
            UserId == IdentificationViewModel.TerritorialBuilderId ||
            UserId == IdentificationViewModel.SecondTerritorialBuilderId ||
            UserId == IdentificationViewModel.ThirdSolidarBuilderId
