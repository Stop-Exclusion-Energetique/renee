...
namespace Renee.UI.Components.Features.Coproperty.Synthesis.OrganizeAndFinance.Modal;

public partial class OrganizeAndFinanceCopropertySynthesisValidationModal
{
    [CascadingParameter] public BlazoredModalInstance BlazoredModal { get; set; } = default!;
    [Parameter] public string AnahAidRequestNotificationFileName { get; set; } = string.Empty;
    [Parameter] public MemoryStream? AnahAidRequestNotificationMemoryStream { get; set; }
    [Parameter] public string GeneralMeetingFileName {  get; set; } = string.Empty;
    [Parameter] public MemoryStream? GeneralMeetingMemoryStream { get; set; }
    ...
    [Parameter] public OrganizeAndFinanceCopropertySynthesisValidationModalViewModel ViewModel { get; set; } = new();
    ...
    private async Task SubmitValidation()
    {
        ...
        if (!Guid.TryParse(userIdString, out var userId) || AnahAidRequestNotificationMemoryStream == null || GeneralMeetingMemoryStream == null) return;

        await FileService.UploadFileAsync(AnahAidRequestNotificationFileName, AnahAidRequestNotificationFileName, AnahAidRequestNotificationMemoryStream, userId);
        await FileService.UploadFileAsync(GeneralMeetingFileName, GeneralMeetingFileName, GeneralMeetingMemoryStream, userId);
        ...
        var stage = IsUserAbleToValidateSynthesisWithoutValidation() ? AccompanyingFileStage.RealisationAndFollowing : AccompanyingFileStage.OrganizingAndFinancing;
