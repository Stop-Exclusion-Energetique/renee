...
namespace Renee.UI.Components.Features.AccompanyingFiles.QuickAdd;

public partial class QuickAddForm
{
	public QuickAddFormViewModel QuickAddFormViewModel { get; } = new();
	...
	private EditContext? _quickAddEditContext;
	...
	private string _userRole { get; set; } = string.Empty;
	...
	{
		_quickAddEditContext = new EditContext(QuickAddFormViewModel);
		...
		_userRole = claims.FirstOrDefault(c => c.Type == ClaimTypes.Role)?.Value ?? string.Empty;
		...
		if (_userRole.Equals(Constants.SolidarBuilderRole)) QuickAddFormViewModel.ReferentSolidarBuilderId = _userId;

		await LoadQuickAddData(IsUserTeritorialBuilderOrCoordinator(_userRole));
	...
	private async Task<bool> OccupantEmailAlreadyExists(string? email) =>
		email is not null && (await MainOccupantService.GetAllMainOccupantAsync()).Value!.Any(o => o.Email == email);

	private async Task<bool> OccupantPhoneNumberAlreadyExists(string? phoneNumber) =>
		phoneNumber is not null &&
		(await MainOccupantService.GetAllMainOccupantAsync()).Value!.Any(o => o.PhoneNumber == phoneNumber);

	private void OnMarkerNatureChanged()
	{
		if (QuickAddFormViewModel.AccompaniementViewModel.MarkerNature is not null && QuickAddFormViewModel.AccompaniementViewModel.MarkerNature == Domain.Enums.MarkerNature.Other)
			return;
	...
	{
		if (_quickAddEditContext is not null && _quickAddEditContext.Validate())
		{
			var isDuplicateOccupants = await OccupantPhoneNumberAlreadyExists(QuickAddFormViewModel.PhoneNumber);

			if (isDuplicateOccupants)
			{
				ShowZeeErrorModal(Labels.Errors.DuplicateOccupant);
				return;
			}

			var isDuplicateAddress = await OccupantEmailAlreadyExists(QuickAddFormViewModel.Email);

			if (isDuplicateAddress)
			{
				ShowZeeErrorModal(Labels.Errors.DuplicateAddress);
				return;
			}

			var solidarBuilderFullName = GetSolidarBuilderFullNameFormated();

			QuickAddFormViewModel.Trigram = AccompanyingFileHelper.GenerateTrigram(QuickAddFormViewModel.FirstName, QuickAddFormViewModel.LastName);

			var mainOccupant = new QuickAddCreatedOccupant(
				QuickAddFormViewModel.Trigram,
				QuickAddFormViewModel.FirstName,
				QuickAddFormViewModel.LastName,
				QuickAddFormViewModel.PhoneNumber,
				QuickAddFormViewModel.Email);
			...
			var householdPropertyType = new QuickAddHousingPropertyType(
				QuickAddFormViewModel.AskedPropertyType,
				QuickAddFormViewModel.CopropertyProfileId);

			var accompanyingFileEntities = new QuickAddCreatedAccompanyingFileEntities(
				mainOccupant,
				housingAddress,
				householdTypology,
				supportTeam,
			...
			var result = await AccompanyingFileService.CreateAccompanyingFileWithQuickAdd(
			...
			if (result.IsSuccess && result.Value is not null)
				NavigationManager.NavigateTo($"{Endpoints.NewOccupant}/{result.Value}");
	...
	private bool ShouldDisplayCopropertyAttachmentField => QuickAddFormViewModel.AskedPropertyType == HousingType.ResidentialCollective;

	private async Task LoadCoproperty()
	{
		var result = await CopropertyProfileService.GetCopropertyProfileList(_userRole, _userId);
	...
	{
		if (QuickAddFormViewModel.AskedPropertyType != HousingType.ResidentialCollective)
		{
			QuickAddFormViewModel.CopropertyProfileId = null;
			_quickAddEditContext!.NotifyFieldChanged(_quickAddEditContext.Field(nameof(QuickAddFormViewModel.CopropertyProfileId)));
		}
		StateHasChanged();
	}
