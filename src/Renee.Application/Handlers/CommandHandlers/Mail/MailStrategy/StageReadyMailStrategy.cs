...
public class StageReadyMailStrategy(IEmailSenderService emailSenderService, IEmailRepository emailRepository) : ISendMailStrategy
{
    public bool CanHandle(MailType type) => type == MailType.StageReady;
    ...
    public async Task<bool> SendAsync(SendMailCommand request, CancellationToken cancellationToken)
    {
        ...
        if (mail is not null && !string.IsNullOrEmpty(mail.Subject.Trim()) && !string.IsNullOrEmpty(mail.HtmlContent.Trim()))
        {
            return await emailSenderService.SendEmail(
					request.Mails,
					string.Format(mail.Subject, request.MailParameters?.Param1, request.MailParameters?.Param2),
					string.Format(
						mail.HtmlContent,
