...
public class InscriptionConfirmationMailStrategy(IEmailSenderService emailSenderService, IEmailRepository emailRepository) : ISendMailStrategy
{
    public bool CanHandle(MailType type) => type == MailType.InscriptionConfirmation;
    ...
    public async Task<bool> SendAsync(SendMailCommand request, CancellationToken cancellationToken)
    {
        ...
        if (mail is not null && !string.IsNullOrEmpty(mail.Subject.Trim()) && !string.IsNullOrEmpty(mail.HtmlContent.Trim()))
        {
            return await emailSenderService.SendEmail(
					request.Mail,
					mail.Subject,
					string.Format(mail.HtmlContent, request.MailParameters?.Param1, request.MailParameters?.Param2));
        }
