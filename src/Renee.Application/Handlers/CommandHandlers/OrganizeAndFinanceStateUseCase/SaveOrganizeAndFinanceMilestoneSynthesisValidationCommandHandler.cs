...
public class SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandHandler(
	IAccompanyingFileRepository accompanyingFileRepository,
	IUserRepository userRepository,
	ITelemetryService telemetryService)
	: IRequestHandler<SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandInput, ReneeOperationResult<RequiredDataForStageSynthesisValidationEmail>>
{
	public async Task<ReneeOperationResult<RequiredDataForStageSynthesisValidationEmail>> Handle(
		SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandInput request,
		CancellationToken cancellationToken)
	{
		try
		{
			...
			accompanyingFile.UpdateSynthesis(request.Milestone, request.Status, request.UserId);

			var result = await accompanyingFileRepository.UpdateAccompanyingFileSynthesis(
				accompanyingFile,
				GetInvoicesFromWorkPackages(accompanyingFile));
	...
	public static List<Invoice> GetInvoicesFromWorkPackages(Domain.Entity.AccompanyingFile accompanyingFile) =>
        [.. accompanyingFile.AccompanyingFilePreWorkPlanNavigation.WorkPackages
		.Select(wp => new Invoice
		{
			AccompanyingFileId = accompanyingFile.Id,
			InvoiceCost = wp.WorkPackageWorkTypeCosts.Sum(wp => wp.Cost)
		})];
}