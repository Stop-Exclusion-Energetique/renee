...
public class SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandTests : TestContext
{
	...
	public async Task Handler_Should_ReturnFalse_WhenAccompanyingFileIdIsEmpty()
	{
		...
		var command = new SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandInput(
			Guid.Empty,
			AccompanyingFileStatus.InProgress,
			AccompanyingFileStage.Identify,
			Guid.NewGuid());

		var handler = new SaveOrganizeAndFinanceMilestoneSynthesisValidationCommandHandler(
